...
Agen8 persists everything under a configurable `dataDir` (default `~/.agen8`)
or `$XDG_STATE_HOME/agen8`. The CLI resolves the directory via `internal/config/effectiveConfig()`
and the `--data-dir` flag / `AGEN8_DATA_DIR` environment variable.
...
Agen8 separates work into sessions and runs. Each session holds a stable goal/context,
...
Agen8 exposes an Agentic File System (AFS) inside each run:

- `/project` – the host working directory (default `.`) mounted via `--workdir` / `AGEN8_WORKDIR`.
...
Understanding this layout makes it easier to inspect sessions/runs manually (e.g., querying `agen8.db`
...
1. `./agen8 list sessions` to find a session ID.
2. `./agen8 show session <id>` or query `agen8.db` to inspect goals.
3. `./agen8 list runs <id>` plus `agen8.db` to review a specific run.
