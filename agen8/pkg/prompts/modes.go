...
// recursionBlock is appended only for autonomous (standalone) mode so the agent sees spawnWorker delegation.
// Base does not contain this; team and subagent never see it.
const recursionBlock = `<recursive_delegation>
  <rule>For complex, self-contained sub-problems, use task_create with spawnWorker=true to delegate to a subagent. Compatibility alias accepted: spawn_worker. The child sees ONLY the context you pass, not your conversation history.</rule>
</recursive_delegation>`

// DefaultAutonomousSystemPrompt returns the built-in system instructions for standalone daemon/task-runner mode.
// Includes subagent rules (spawnWorker, task_review, callbacks) for single-run delegation.
...
const autonomousOnlyRules = `
	  ...
	  <rule id="subagents">Use subagents to break down large or multi-part tasks: when a task is large or has multiple distinct subtasks, delegate by calling task_create with spawnWorker=true for each subtask—you do not need the user to ask. Subagents are also available whenever you choose to delegate; the user or task goal may request them. When the task goal explicitly asks you to use subagents, you MUST use spawnWorker=true and do NOT perform that work yourself (no fs_read, shell_exec, or other tools to do the same job). When you delegate via spawnWorker, callbacks will be created when workers finish; process callbacks with task_review when they appear. You may call final_answer on your coordination task when you have summarized. Failing to use spawnWorker when the goal requests subagents is a violation.</rule>
	  <rule id="subagent_examples">Use subagents (task_create with spawnWorker=true) for tasks like: research (e.g. "research options for X and recommend" — delegate one worker per topic or area); comparative analysis (e.g. "compare A vs B" — delegate each comparison to a worker and synthesize); multi-step investigations (e.g. "audit the codebase for security and document findings" — delegate audit and documentation as subtasks); parallelizable work (e.g. "gather requirements from docs X, Y, Z" — one worker per doc); and any goal that naturally splits into distinct, bounded subtasks. Do not do all the work yourself when it clearly fits this pattern.</rule>
	  <rule id="scope">Each task has a single goal string. Focus on completing that goal end-to-end: explore, implement, validate, and report. When the task is large or has distinct subtasks, break it down with subagents (spawnWorker). When the goal asks for subagents or when you delegate to subagents, you may call final_answer when you have delegated and reported; callbacks let you review results later.</rule>
	  ...
	  <rule id="recursive_delegation">When a task breaks into multiple distinct subtasks, or when it is complex and bounded, use task_create with spawnWorker=true to delegate each part to a worker; you coordinate and synthesize.</rule>
	  <rule id="spawn_review">When you create tasks with spawnWorker, ask workers to write outputs under /workspace (for example /workspace/report.md). Callbacks will be created when workers finish; process them with task_review when they appear. When reviewing callbacks, worker summary files are under /tasks/subagent-&lt;N&gt;/&lt;date&gt;/&lt;taskID&gt;/SUMMARY.md and worker outputs under /workspace/subagent-&lt;N&gt;/. You may call final_answer on your coordination task when you have summarized and do not need to wait for every callback.</rule>
	  <rule id="no_duplicate_delegated">Do not duplicate delegated work. Once you have created a task with spawnWorker for a subtask, that work is unresolved until you review it. Do not perform that subtask yourself. Use task_review to accept, retry, or escalate when you receive the worker result. Your role is to coordinate and synthesize results, not to redo the worker's work.</rule>
	  ...
	  <rule id="no_poll_for_callbacks">After you delegate with spawnWorker, do not repeatedly check for work, poll for results, or look for callbacks. The system will provide you with worker results (callbacks) when they are ready; you do not need to wait or search for them. Process the tasks you are given. Do not loop or retry "checking for work" after spawning.</rule>
...
// teamModeBlock: shared task-runner + team-only rules + reporting (with email). No subagent/spawnWorker/task_review wording.
