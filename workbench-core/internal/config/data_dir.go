...
const (
	// EnvDataDir overrides the workbench data directory.
	EnvDataDir = "WORKBENCH_DATA_DIR"

	// EnvXDGStateHome is the XDG base directory for state data.
	// If set, workbench defaults to "$XDG_STATE_HOME/workbench".
	EnvXDGStateHome = "XDG_STATE_HOME"
)

// ResolveDataDir resolves the workbench DataDir used by the host runtime.
//
// Precedence:
//  1. CLI flag (only if explicitly set)
//  2. env var WORKBENCH_DATA_DIR
//  3. default: ~/.workbench (or $XDG_STATE_HOME/workbench when XDG_STATE_HOME is set)
//
...
func defaultDataDir() (string, error) {
	if xdg := strings.TrimSpace(os.Getenv(EnvXDGStateHome)); xdg != "" {
		return filepath.Join(xdg, "workbench"), nil
	}
	...
	return filepath.Join(home, ".workbench"), nil
}
...
func ensureDirWritable(dir string) error {
	...
	f, err := os.CreateTemp(dir, ".workbench_write_test_*")
	if err != nil {
