...
type ResultsStore interface {
	// PutCall stores the response.json payload for a tool call.
	//
	// callID must be a non-empty identifier generated by the host/runner.
	// responseJSON is the exact bytes served at "/results/<callId>/response.json".
	PutCall(callID string, responseJSON []byte) error

	// GetCallResponseJSON returns the response.json bytes for a call ID.
	GetCallResponseJSON(callID string) ([]byte, error)

	// ListCallIDs returns all known call IDs, sorted for stable listing.
	ListCallIDs() ([]string, error)

	// PutArtifact stores a tool artifact for a call.
	//
	// artifactPath is the path relative to "/results/<callId>/" (e.g. "stdout.txt").
	PutArtifact(callID, artifactPath, mediaType string, content []byte) error

	// GetArtifact reads a stored artifact.
	//
	// Returns (content, mediaType, error).
	GetArtifact(callID, artifactPath string) ([]byte, string, error)

	// ListArtifacts returns metadata for artifacts under a call, sorted by path.
