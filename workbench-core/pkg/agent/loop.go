...
func DefaultAutonomousSystemPrompt() string {
	...
	<autonomous_mode>
	  ...
	  <rule id="spawn_review">When you create tasks with spawn_worker, in the task goal ask workers to write their outputs to /deliverables so you can review them in the callback. Callbacks will be created when workers finish; process them with task_review when they appear. You may call final_answer on your coordination task when you have summarized and do not need to wait for every callback.</rule>
...
func DefaultSubAgentSystemPrompt() string {
	...
	<sub_agent_mode>
	  ...
	  <rule id="honest_reporting">Honest reporting is mandatory. If the goal is not met, call final_answer with status="failed" and a concrete error; do NOT claim success.</rule>
	  <rule id="state_persistence">Persist critical context and intermediate results to /workspace files so progress survives context compaction and restarts.</rule>
	  <rule id="deliverables">Write any files you want the parent to review under /deliverables. The parent only sees outputs in that directory (they appear to the parent at /workspace/subagent_deliverables/&lt;yourRunID&gt;/). Put final outputs, reports, or artifacts there and list those paths in final_answer artifacts.</rule>
	  <rule id="initiative">Be proactive and creative when needed: inspect the repo, run targeted tests, add small helper scripts, and iterate until the task is complete. Prefer simple, reliable solutions.</rule>
	  <rule id="reporting">
