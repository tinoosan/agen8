...
func TestDefaultAgent_CompactConversationForBudget_UsesServerCompaction(t *testing.T) {
	...
	got := a.compactConversationForBudget(context.Background(), msgs, "system", 128)
	if len(got) != 2 {
		t.Fatalf("len(got)=%d, want 2", len(got))
	}
	if got[1].Role != "compaction" || got[1].Content != "enc" {
...
func TestDefaultAgent_CompactConversationForBudget_FallsBackToLocalCompaction(t *testing.T) {
	...
	got := a.compactConversationForBudget(context.Background(), msgs, "system", 1024)
	foundNotice := false
	for _, m := range got {
