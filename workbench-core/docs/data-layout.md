...
- **Runs** are stored in SQLite (`runs` and `events` tables), while run directories remain on disk:
  ...
  - `user_profile/` – host-level user profile data injected into runs.
...
- `/log`, `/user_profile`, `/tools` – read-only mounts that surface logs, global user profile state, and discovered tool manifests (`internal/tools`).
...
| `/user_profile`   | `<dataDir>/user_profile`                | Global user profile state (durable preferences/facts).             |
