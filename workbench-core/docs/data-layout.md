...
- `/log`, `/profile`, `/tools` – read-only mounts that surface logs, global profile state, and discovered tool manifests (`internal/tools`).
...
| `/profile`   | `<dataDir>/profile`                | Profiling data from the host.                                      |
