...
This document explains how Workbench resolves runtime configuration, how first-run onboarding works, and how to run reliably in server/headless environments.

## Where `config.toml` lives

Default path:

- `${WORKBENCH_DATA_DIR}/config.toml`
- If `WORKBENCH_DATA_DIR` is not set, Workbench uses its normal data-dir default.

Workbench auto-creates this file on startup if missing. It only stores non-secret settings.
...
2. OS keychain (`service=workbench`, account `<provider>.api_key`, default provider `openrouter`)
3. If still missing:
...
Workbench checks both stdin and stdout:
...
3. Start Workbench daemon normally.
...
2. Confirm API key source:
   - env var present, or
   - keychain entry exists (`workbench` / `openrouter.api_key`).
