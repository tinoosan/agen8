...
| ----------------------- | ------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `--data-dir`            | `WORKBENCH_DATA_DIR`            | `~/.workbench` or `$XDG_STATE_HOME/workbench` | Base directory for `sessions`, `runs`, `tools`, `user_profile`, `memory`, `agent`. Overrides where persistent data resides. |
...
| `--user-profile-bytes`  | —                               | `4*1024`                                      | Budget for user profile context injected from `/user_profile`.                                                         |
