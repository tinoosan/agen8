...
How to check that subagent **review gate**, **cleanup**, and **artifact paths** work as expected. This aligns with the [Execution model (PRD)](execution-model.md): sub-agent work is not complete until the parent approves; cleanup happens after approval (or final failure / escalation resolution) and must preserve audit history and indexed artifacts.
