...
func (r *SQLiteRepository) DeleteMission(ctx context.Context, missionID mission.MissionID) error {
	missionID = mission.MissionID(strings.TrimSpace(string(missionID)))
	if missionID == "" {
		return fmt.Errorf("mission id is required")
	}
	...
	for _, stmt := range stmts {
		if _, err := tx.ExecContext(ctx, stmt.query, string(missionID)); err != nil {
