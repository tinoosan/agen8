...
func NewPostgresRepository(handle *storagedb.Handle) (*PostgresRepository, error) {
	if handle == nil || handle.DB() == nil {
		return nil, fmt.Errorf("task postgres repository: db handle is required")
	}
	if handle.Driver() != storagedb.DriverPostgres {
		return nil, fmt.Errorf("task postgres repository: storage driver must be postgres, got %q", handle.Driver())
	}
...
func (r *PostgresRepository) GetTask(ctx context.Context, taskID domain.TaskID) (domain.Task, error) {
...
func (r *PostgresRepository) ListTasks(ctx context.Context, filter domain.TaskFilter) ([]domain.Task, error) {
...
func (r *PostgresRepository) CountTasks(ctx context.Context, filter domain.TaskFilter) (int, error) {
	where, args, err := taskWhere(filter)
	if err != nil {
		return 0, err
	}
	var count int
	if err := r.db.QueryRowContext(ctx, r.rebind("SELECT COUNT(*) FROM tasks"+where), args...).Scan(&count); err != nil {
		return 0, fmt.Errorf("count tasks: %w", err)
	}
	return count, nil
}

func (r *PostgresRepository) saveTask(ctx context.Context, task domain.Task) error {
