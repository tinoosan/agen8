...
// stores spaces, runs, and runtime artifacts. This directory is resolved
...
//
//   - `<dataDir>/spaces/` – metadata about each space (for resuming + listing)
//   - `<dataDir>/agen8.db` – primary SQLite store (spaces, runs, tasks, events, plans, user/auth, etc.)
//   - `<projectRoot>/workspace/` – project-scoped workspace for space member runs (not under DataDir)
//   - `<dataDir>/memory/` – shared memory (`MEMORY.MD`, plus daily `YYYY-MM-DD-memory.md` files)
//   - `<dataDir>/agent/` – upcoming runtime stores that will continue to live under `DataDir`.
//
// # Extension Points
//
