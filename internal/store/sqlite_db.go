...
var sqliteMigrations = []string{
	...
	`CREATE TABLE IF NOT EXISTS project_teams (
		...
		UNIQUE(project_root, team_id)
	);`,
	`CREATE INDEX IF NOT EXISTS idx_project_teams_root_status ON project_teams(project_root, status);`,
	`CREATE INDEX IF NOT EXISTS idx_project_teams_root_updated ON project_teams(project_root, updated_at DESC);`,
}
...
func sqliteMaxOpenConns() int {
	return envInt("AGEN8_SQLITE_MAX_OPEN_CONNS", defaultSQLiteMaxOpenConns)
}

func sqliteMaxIdleConns() int {
	return envInt("AGEN8_SQLITE_MAX_IDLE_CONNS", defaultSQLiteMaxIdleConns)
}

func sqliteConnMaxLifetime() time.Duration {
	return envDuration("AGEN8_SQLITE_CONN_MAX_LIFETIME", defaultSQLiteConnMaxLife)
}
