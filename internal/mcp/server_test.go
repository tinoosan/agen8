...
func TestServer_InitializeDoesNotAdvertiseInboxResources(t *testing.T) {
	store := NewTokenStore()
	store.Register("token-no-resource-init", Session{
		SpaceID:  "space-source",
		MemberID: "member-source",
	})
	server, err := NewServer(store)
	...
	resp := postMCPRequest(t, server.Handler(), "token-no-resource-init", map[string]any{
		"jsonrpc": "2.0",
		"id":      "init-1",
		"method":  "initialize",
		"params": map[string]any{
	...
	if _, ok := result.Capabilities["resources"]; ok {
		t.Fatalf("resources capability should not be advertised: %+v", result.Capabilities)
	}
}
