...
func mustSchema() json.RawMessage {
	body, err := json.Marshal(map[string]any{
		...
		"properties": map[string]any{
			...
			"file_name":           stringSchema("Attachment file name for action=attach. A bare name like build-screenshot.png; path separators are rejected. The file is stored under /project/.agen8/attachments/<task-id>/ and appended to the task's artifacts as file:<vpath>. Attach is for material that does not already live in the project (screenshots, generated reports); a file already in the project tree should be referenced in artifacts as file:<its real vpath> instead of uploading a copy."),
