...
func (s stubRunner) ResumeTurn(ctx context.Context, toolOutputs []llmtypes.LLMMessage) (string, error) {
	_ = toolOutputs
...
func (r blockingRunner) ResumeTurn(ctx context.Context, toolOutputs []llmtypes.LLMMessage) (string, error) {
	_ = toolOutputs
...
func TestLayout_WithActivitySidebar_DoesNotClipHeader(t *testing.T) {
	...
	if !strings.HasPrefix(view, "agen8") {
		t.Fatalf("expected header to start with %q, got prefix %q", "agen8", view[:min(len(view), 32)])
	}
}
...
func TestLayout_NarrowTerminal_HeaderStillVisibleWhenFooterWraps(t *testing.T) {
	...
	if !strings.Contains(firstLine, "agen8") {
		t.Fatalf("expected header line to contain %q, got %q", "agen8", firstLine)
	}
}
...
func TestFilePicker_OpensOnAtAndFiltersFromInput(t *testing.T) {
	tmp, err := os.MkdirTemp("", "agen8-filepicker-*")
	if err != nil {
