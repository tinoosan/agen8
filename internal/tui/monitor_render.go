...
func (m *monitorModel) renderDashboard(grid layoutmgr.GridLayout, headerLine string) string {
	...
	if m.isDetached() {
		...
		if m.rpcHealthKnown && !m.rpcReachable {
			warningText = "Daemon disconnected at " + strings.TrimSpace(m.rpcEndpoint) + ". Start `agen8 daemon` and retry with /reconnect."
		}
...
func (m *monitorModel) renderHeader() string {
	...
	if m.isDetached() {
		...
		content = lipgloss.JoinHorizontal(lipgloss.Left,
			m.styles.headerTitle.Render("Agen8 - Always On "),
