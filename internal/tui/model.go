...
func New(ctx context.Context, runner TurnRunner, evCh <-chan events.Event) Model {
	...
	// For Agen8, we want focus to affect behavior (cursor + key handling) but not
	...
	m := Model{
		...
		// Ctrl+A, or enable it by default via AGEN8_ACTIVITY/--activity.
		showDetails:         false,
...
// Run starts the Agen8 Bubble Tea program.
func Run(ctx context.Context, runner TurnRunner, evCh <-chan events.Event) error {
	...
	//   - env: AGEN8_ACTIVITY=true/false
	//   - flag: --activity (wired in cmd/agen8)
	enableActivity := strings.TrimSpace(os.Getenv("AGEN8_ACTIVITY"))
	if enableActivity != "" {
	...
	// selection unless your terminal supports shift-drag selection. Agen8 defaults
	// to mouse scrolling; set AGEN8_MOUSE=false (or --mouse=false) to restore
	// native selection behavior.
	//
	// Mouse mode is opt-in via:
	//   - env: AGEN8_MOUSE=true/false
	//   - flag: --mouse (wired in cmd/agen8)
	enableMouse := strings.TrimSpace(os.Getenv("AGEN8_MOUSE"))
	mouseOn := true
	if enableMouse != "" {
