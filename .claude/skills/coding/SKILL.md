...
- If a change risks breaking other areas, document expected impacts in the summary and add tests that cover the risk — do not add feature flags, fallback values, or silent degradation paths to mask the risk.
- When in doubt about requirements, surface the ambiguity and ask for clarification. Never guess a "safe default" — missing requirements must be resolved, not papered over with fallback values.
- Follow the Boy Scout Rule: leave code cleaner than you found it (within reason).
- **No silent fallbacks.** Code must fail loudly when something is wrong. Never substitute a default value for a missing input, silently swallow an error, or degrade to a no-op. If a function cannot succeed, it must return an error.
- **Test failure paths.** When a function can fail, write a test that triggers the failure and asserts correct error behavior. Untested error paths are where hidden fallbacks accumulate.
...
- **Silent fallbacks**: Don't substitute default values for missing inputs, swallow errors, or degrade to no-ops. These hide bugs and make debugging impossible. If something is wrong, fail loudly.
- **Defensive over-recovery**: Don't wrap code in try/catch or `if err != nil { return defaultValue }` just to keep things running. A crash you can see is better than a bug you can't.
