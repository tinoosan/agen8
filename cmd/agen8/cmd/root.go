...
`),
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		if enableMouse {
			_ = os.Setenv("AGEN8_MOUSE", "true")
		} else {
			_ = os.Unsetenv("AGEN8_MOUSE")
		}

		// Activity panel is opt-in (default closed).
		if enableActivity {
			_ = os.Setenv("AGEN8_ACTIVITY", "true")
		} else {
			_ = os.Unsetenv("AGEN8_ACTIVITY")
		}
...
func init() {
	rootCmd.PersistentFlags().StringVar(&dataDir, "data-dir", "", "base directory for agents/sessions (priority: --data-dir, env AGEN8_DATA_DIR, default: ~/.agen8 or $XDG_STATE_HOME/agen8)")
	workDir = strings.TrimSpace(os.Getenv("AGEN8_WORKDIR"))
	rootCmd.PersistentFlags().StringVar(&workDir, "workdir", workDir, "host working directory to mount at /project (default: current directory; env AGEN8_WORKDIR)")
	rootCmd.PersistentFlags().IntVar(&maxContextB, "context-bytes", 8*1024, "run.maxBytesForContext (persisted in run.json)")
	enableMouse = envBool("AGEN8_MOUSE", false)
	...
	enableActivity = envBool("AGEN8_ACTIVITY", false)
	rootCmd.PersistentFlags().BoolVar(&enableActivity, "activity", enableActivity, "show activity panel by default (env AGEN8_ACTIVITY)")
	protocolStdio = envBool("AGEN8_PROTOCOL_STDIO", true)
	...
	rpcEndpoint = strings.TrimSpace(os.Getenv("AGEN8_RPC_ENDPOINT"))
	if rpcEndpoint == "" {
	...
	rootCmd.PersistentFlags().StringVar(&resultWebhookURL, "result-webhook-url", resultWebhookURL, "POST task results to this webhook URL (env AGEN8_RESULT_WEBHOOK_URL)")
	healthAddr = strings.TrimSpace(os.Getenv("AGEN8_HEALTH_ADDR"))
	rootCmd.PersistentFlags().StringVar(&healthAddr, "health-addr", healthAddr, "listen address for health checks (env AGEN8_HEALTH_ADDR)")
	...
	includeHistoryOps = envBool("AGEN8_INCLUDE_HISTORY_OPS", true)
