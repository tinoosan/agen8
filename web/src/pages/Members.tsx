 ...
 * The added Performance column pushes the table to eight columns, so the switch
 * sits at ≥860px (and the table itself scrolls horizontally as a last resort) —
 * the table also returns on its own if the sidebar collapses to its icon rail. */
...
function MemberRoster(props: {
  ...
  return (
    <div className="@container">
      <div className="flex flex-col gap-3 @min-[860px]:hidden">
        {props.members.map((m) => (
          <MemberCard
            key={m.id}
            m={m}
...
function MemberTable({
  ...
  return (
    <div className="hidden overflow-x-auto rounded-[var(--r-lg)] border border-[var(--border)] bg-[var(--bg-surface)] @min-[860px]:block">
